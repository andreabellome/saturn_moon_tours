function [st_man_prop] = heteroclinic_process_stable_manifold(st_man_prop, strucNorm)

% --> START: prune at the Poincare section
for ii = 1:length(st_man_prop)
    
    sv_complete_uns = st_man_prop(ii).yy;
    tt_complete_uns = st_man_prop(ii).tt;
    ...
    st_man_prop(ii).sv_int = sv_uns_mod;
    st_man_prop(ii).t_int  = t_uns_mod;
end
...
for ii = 1:length(st_man_prop)
    ...
    sv_uns_L1_int = st_man_prop(ii).sv_int;
    t_uns_L1_int  = st_man_prop(ii).t_int;
    ...
    if min(save_index) == 0
        % --> prune this
        st_man_prop(ii).sv_int = [];
        st_man_prop(ii).t_int  = [];
    end
