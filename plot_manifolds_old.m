function fig = plot_manifolds_old( manifolds, strucNorm, manifold_plot, holdon )
    ...
    for indez = 1:size(sv0_st, 1)
        ...
        if ~isempty(yy1)
            if indez == 1
                ...
                plot3( yy1(end,1).*normDist, yy1(end,2).*normDist, yy1(end,3).*normDist, 'o', 'MarkerFaceColor', 'yellow', 'HandleVisibility', 'Off' );
            end
        end
