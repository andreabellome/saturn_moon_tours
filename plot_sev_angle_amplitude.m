function [fig] = plot_sev_angle_amplitude( strucOrbit, strucNorm, holdon, colors)
...
xlabel( 'time/Period' ); ylabel( 'SEV angle [deg]' );
hold on; grid on;

for index = 1:length(strucOrbit)
    plot( strucOrbit(index).tt./strucOrbit(index).norm_plot, rad2deg( strucOrbit(index).sev_angle ), ...
        'Color', colors(index,:), ...
        'LineWidth', 2, ...
